  ...
  return (
    <section id="about" className="about-section bg-[#FDFEFF] w-full py-[50px] md:py-[40px] scroll-mt-[80px] px-5 md:px-[5%]">
      <div className="about-container max-w-[1280px] mx-auto bg-[#FDFEFF]">
        ...
        <div 
          ref={wrapperRef}
          className="about-images-wrapper w-full overflow-hidden mb-6 md:mb-10 relative"
          style={{
        ...
        >
          <div className="about-images-scroll w-full overflow-hidden">
            <div 
              ref={galleryRef}
              className="about-images flex gap-4 w-max"
              style={{
            ...
            >
              {allImages.map((image, idx) => (
                <Image
                  ...
                  width={240}
                  height={180}
                  className="about-gallery-img w-[240px] min-w-[240px] h-[180px] rounded-xl object-cover shrink-0 cursor-pointer transition-all hover:scale-105 hover:shadow-[0_12px_32px_rgba(18,52,91,0.2)] hover:z-10 hover:relative"
                  loading="lazy"
                />
              ))}
            </div>
        ...
        {/* STATS */}
        <div className="stats-wrapper flex flex-wrap gap-3.5 md:gap-6 mb-5 md:mb-8 justify-start items-stretch">
          {stats.map((stat, idx) => (
            <div 
              key={idx} 
              className="stat-box flex-1 min-w-[120px] max-w-[140px] md:min-w-[150px] p-4 md:p-[26px_30px] rounded-xl border-2 border-[#cfe0ff] text-center bg-[#f0f7ff] transition-all flex flex-col items-center justify-center shadow-[0_2px_8px_rgba(18,52,91,0.04)] hover:bg-[#e6f2ff] hover:border-[#a8c8ff] hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(18,52,91,0.08)]"
            >
              <h3 className="text-2xl md:text-[28px] font-bold text-[#0a65d9] mb-1.5 leading-none tracking-[-0.3px]">{stat.value}</h3>
              <p className="text-[13px] md:text-[15px] font-medium text-[#666] m-0 leading-[1.3]">{stat.label}</p>
            </div>
          ))}
        </div>

        {/* DESCRIPTION */}
        <p className="about-text max-w-[650px] text-[15px] md:text-[17px] leading-[1.6] md:leading-[1.65] text-[#444] mt-0 mb-0">
